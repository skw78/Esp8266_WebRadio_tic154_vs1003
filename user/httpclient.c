...
struct espconn * mp3_conn = NULL;
extern  char   mp3_callback(char * buf, unsigned short len); // kostyl :)
...
//	char * new_buffer;
//	if (new_size > BUFFER_SIZE_MAX || NULL == (new_buffer = (char *)os_zalloc(new_size))) {
...
//	char buf[69 + strlen(method) + strlen(req->path) + strlen(req->hostname) +
	char buf[128 + strlen(method) + strlen(req->path) + strlen(req->hostname) +
...
//  os_sprintf(data,"GET /esp8266/esp_02.php?mode=%u&submode=%u HTTP/1.0\r\n\r\n\r\n",mode,submode);
//	if (req->secure)
